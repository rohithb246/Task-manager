...
<<<<<<< HEAD
        ...
        actual_role = "admin" if user.is_staff else "intern"
        if expected_role and expected_role != actual_role:
            label = "Admin" if actual_role == "admin" else "Intern"
            return Response(
...
<<<<<<< HEAD
        if status_filter == "Completed":
            ...
            queryset = queryset.filter(Q(submission_file__isnull=True) | Q(submission_file=""))
        elif status_filter == "In Progress":
            queryset = queryset.none()
        if priority:
            queryset = queryset.filter(priority=priority)
        open_submission_filter = Q(submission_file__isnull=True) | Q(submission_file="")
        if deadline == "overdue":
            queryset = queryset.filter(open_submission_filter, deadline__lt=timezone.now())
...
<<<<<<< HEAD
    ...
    status_counts = {"Pending": pending, "In Progress": 0, "Completed": completed}
